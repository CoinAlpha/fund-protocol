...
contract('OwnableModified', (accounts) => {
  ...
  before('should prepare', () => {
    ...
    [owner0, owner1, owner2, owner3, notOwner0, notOwnerAddress0, notOwnerAddress1, notOwnerAddress2, notOwnerAddress3] = accounts;
  });

  Object.keys(constructors).forEach((name) => {
    describe(name, () => {
      beforeEach(`should deploy a new ${name}`, () => {
        constructors[name](owner0, notOwnerAddress0, notOwnerAddress1, notOwnerAddress2)
      ...
      describe('getOwners', () => {
        ...
        it('should be possible to ask for owner from any address', () => {
          owned.getOwners({ from: notOwner0 })
            .then(owners => assert.strictEqual(owners[0], owner0));
        });

        it('should be possible to send a transaction to getOwner', () => {
          owned.getOwners.sendTransaction({ from: owner1 })
        ...
        it('should not be possible to send a transaction with value to getOwners', () => {
          owned.getOwners.sendTransaction({ from: owner1, value: 1 })
            .then(
      ...
      describe('addOwner', () => {
        it('should be possible to add another owner', () => {
          owned.addOwner(owner1, { from: owner0 })
        ...
        it('should not be possible to add a third owner', () => {
          owned.addOwner(owner1, { from: owner0 })
            .then(txObj => web3.eth.getTransactionReceiptMined(txObj.tx))
            .then((receipt) => {
              assert.strictEqual(receipt.logs.length, 1);
      ...
      describe('transferOwnership', () => {
        ...
        it('should not be possible to set owner if pass value', () => {
          owned.transferOwnership(owner2, { from: owner0, value: 1 })
            .then(
              () => assert.throw('should not have reached here'),
              e => assert.isAtLeast(e.message.indexOf('non-payable function'), 0)
            );
        ...
        it('should be possible to transfer ownership', () => {
          owned.transferOwnership.call(owner1, { from: owner0 })
  ...
  it('should have correct number of functions', () => {
    constructors.OwnableModified(owner0)
