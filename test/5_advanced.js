...
contract('Advanced', (accounts) => {
  ...
  before(() => DataFeed.new(
    ...
    .then(() => {
      Promise.all(investors.map(acct => fund.modifyAllocation(acct, ethToWei(30))));
    })
  ...
  beforeEach((done) => {
    ...
    Promise.all(investors.map(acct => fund.requestSubscription({ from: acct, value: ethToWei(MIN_INITIAL_SUBSCRIPTION) })))
      .then(() => {
        ...
        Promise.all(investors.map(acct => fund.getInvestor(acct)));
      })
      .then((_values) => {
  ...
  it('should redeem all redemption requests', (done) => {
    fund.remitFromExchange({ from: EXCHANGE, value: ethToWei(99), gas: GAS_AMT })
      .then(() => {
        Promise.all(investors.map(acct => fund.requestRedemption(MIN_REDEMPTION_SHARES, { from: acct })));
      })
