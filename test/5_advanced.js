...
contract('Advanced', (accounts) => {
  ...
  before(() => DataFeed.new(
    ...
    .then(() => Promise.all(investors.map(acct => fund.modifyAllocation(acct, ethToWei(30)))))
  ...
  beforeEach((done) => {
    ...
    Promise.all(investors.map(acct => fund.requestSubscription({ from: acct, value: ethToWei(MIN_INITIAL_SUBSCRIPTION) })))
      .then(() => Promise.all(investors.map(acct => fund.getInvestor(acct))))     // Gas for subscribing a single investor ~=81800
      .then((_values) => {
        _values.forEach((val) => {
      ...
      .then(() => Promise.all(investors.map(acct => fund.getInvestor(acct))))
      .then((_values) => {
        _values.forEach((val) => {
  ...
  it('should redeem all redemption requests', (done) => {
    fund.remitFromExchange({ from: EXCHANGE, value: ethToWei(99), gas: GAS_AMT })
      .then(() => Promise.all(investors.map(acct => fund.requestRedemption(MIN_REDEMPTION_SHARES, { from: acct }))))
